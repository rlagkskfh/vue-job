...
const { isLogin, user, checkLoginStatus } = useAuth(); // 로그인 상태 확인
const router = useRouter()
// const isLogin = ref(false); // 화면 표시 상태 변수

const name = ref('Creator_King_YESUA_CHRIST');
const addr = ref('Haven');
const text = ref('예수께서 이르시되 내가 곧 길이요 진리요 생명이니 나로 말미암지 않고는 아버지께로 올 자가 없느니라 (요 14:6)')
